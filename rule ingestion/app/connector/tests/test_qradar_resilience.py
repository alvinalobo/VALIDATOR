...
def create_qradar_connector():
    config = ConnectorConfig(
        connector_id="qradar-resilience-test",
        vendor="qradar",
        product="siem",
        credentials={
            "base_url": "http://qradar-test",
...
def test_query_retries_then_uses_fallback(monkeypatch):
    connector = create_qradar_connector()
...
def test_circuit_opens_after_repeated_qradar_failures(monkeypatch):
    connector = create_qradar_connector()
...
def test_open_circuit_blocks_qradar_request(monkeypatch):
    connector = create_qradar_connector()
    ...
    # Second request must be blocked by the open circuit.
    result = connector.query(
        "SELECT * FROM events",
        time_range=(None, None),
    )
...
def test_reset_allows_qradar_requests_again(monkeypatch):
    connector = create_qradar_connector()
